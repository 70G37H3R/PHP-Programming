...
class Appointment
{
	...
	{
		$this->connect = new PDO("mysql:host=localhost;dbname=php_hospital", "root", "");
	...
	function Generate_prescription_no()
	{
		$this->query = "
		SELECT MAX(prescription_no) as prescription_no FROM prescription 
		";
		...
		foreach($result as $row)
		{
			$prescription_number = $row["prescription_no"];
		}

		if($prescription_number > 0)
		{
			return $prescription_number + 1;
		}
		else
		{
			return '1000';
	...
	function get_total_today_prescription()
	{
		$this->query = "
	...
	function get_total_yesterday_prescription()
	{
		$this->query = "
		SELECT * FROM prescription 
	
		WHERE prescription_date = CURDATE() - 1
		";
		$this->execute();
		return $this->row_count();
	}

	function get_total_seven_day_prescription()
	{
		$this->query = "
		SELECT * FROM prescription 
	
		WHERE prescription_date >= DATE(NOW()) - INTERVAL 7 DAY
		";
		$this->execute();
		return $this->row_count();
	}
