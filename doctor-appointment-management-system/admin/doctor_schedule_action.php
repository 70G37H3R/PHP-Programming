...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'fetch')
	{
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = $row["average_consulting_time"] . ' Phút';

			$status = '';
			if($row["doctor_schedule_status"] == 'Active')
			{
				$status = '<button type="button" name="status_button" class="btn btn-primary btn-sm status_button" data-id="'.$row["doctor_schedule_id"].'" data-status="'.$row["doctor_schedule_status"].'">Kích Hoạt</button>';
			}
			else
			{
				$status = '<button type="button" name="status_button" class="btn btn-danger btn-sm status_button" data-id="'.$row["doctor_schedule_id"].'" data-status="'.$row["doctor_schedule_status"].'">Chặn</button>';
			}
