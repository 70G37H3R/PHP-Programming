...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'fetch')
	{
		...
		foreach($result as $row)
		{
			...
			{
				$status = '<button type="button" name="status_button" class="btn btn-primary btn-sm status_button" data-id="'.$row["doctor_id"].'" data-status="'.$row["doctor_status"].'">Kích Hoạt</button>';
			}
			else
			{
				$status = '<button type="button" name="status_button" class="btn btn-danger btn-sm status_button" data-id="'.$row["doctor_id"].'" data-status="'.$row["doctor_status"].'">Chặn</button>';
			}
			$sub_array[] = $status;
			$sub_array[] = '
			<div align="center">
	...
	if($_POST["action"] == 'Add')
	{
		...
		else
		{
			...
			if($_FILES['doctor_profile_image']['name'] != '')
			{
			    ...
			    {
			        $error = "<div class='alert alert-danger'>Chọn ảnh có định dạng. jpg, png</div>";
			    }
			    else if (($_FILES["doctor_profile_image"]["size"] > 2000000))
			    {
			       $error = "<div class='alert alert-danger'>Kích thước ảnh phải nhỏ hơn 2MB</div>";
			    }
			    else
			    {
	...
	if($_POST["action"] == 'fetch_single')
	{
		...
		foreach($result as $row)
		{
			...
			$data['doctor_added_on'] = '<span class="badge badge-success">'.date('d-m-Y', strtotime($row['doctor_added_on'])).'</span>';
